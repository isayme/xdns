...
{
    if (R_ERROR == xdns_check_para(argc, argv))
    {
    ...
    if (R_ERROR == udp_init(XDNS_DNS_PORT, xdns_callback))
    {
    ...
    while ((ch = getopt(argc, argv, ":d:c:l:t:h")) != -1)
    {
        switch (ch)
        {
            ...
            case 'c':
                ...
                if (0 != access((char *)(g_config.cfg_path), R_OK))
                {
...
static INT32 xdns_get_config()
{
    ...
    if (R_ERROR == get_cfg_from_file(key, value, XDNS_SRV_ADDR_LEN, (char *)g_config.cfg_path))
    {
    ...
    for (i = 0; i < g_dns_srv.num; i++)
    {
        ...
        if (R_ERROR == get_cfg_from_file(key, g_dns_srv.srvs[i].addr, XDNS_SRV_ADDR_LEN, (char *)g_config.cfg_path))
        {
            PRINTF(LEVEL_ERROR, "get config {%s} error.\n", key);
            free(g_dns_srv.srvs);
            return R_ERROR;
...
static int xdns_callback(struct sockaddr addr, char *buff, int blen)
{
    ...
    while (R_ERROR == tp_add_task(g_threadpool, xdns_tp_func, (void *)tp_arg))
    {
